...
def test_unusable_metrics_fail_closed():
    ...
    nan = float("nan")
    assert decide({"w-greedy": "да", "gigaam": ""}, None, {"no_speech": nan, "peak_db": nan})["status"] == "review"
...
def test_a_confirmed_pair_stops_being_proposed_again():
    assert proposed_terms("внутри юнити проекта", "внутри unity проекта", {"юнити": ["unity"]}) == []


def test_a_cyrillic_word_difference_is_never_forgiven():
    # "пересчитать" vs "перечитать" is a real difference; a human decides it.
    verdict = decide({"w-greedy": "предлагаю пересчитать документ", "gigaam": "предлагаю перечитать документ"})
    assert verdict["status"] == "review"
...
def test_one_gigaam_head_failing_is_not_a_dead_file():
    ...
    verdict = decide({"w-greedy": "привет мир", "gigaam": None, "gigaam-ctc": "привет мир"}, None, SPEECH)
    assert verdict["status"] == "accepted"
...
def test_a_review_verdict_points_at_each_disputed_spot():
    ...
    verdict = decide({"w-greedy": "раз два три четыре пять", "gigaam": "раз два сто четыре пять"}, None, SPEECH)
    assert verdict["status"] == "review"
...
def test_scattered_disagreements_become_separate_spots():
    ...
    verdict = decide(
        {
            "w-greedy": "альфа два три четыре пять шесть семь восемь омега",
...
def test_neighbouring_disagreements_share_one_spot():
    ...
    verdict = decide({"w-greedy": "раз два три четыре пять", "gigaam": "раз сто сорок четыре пять"}, None, SPEECH)
    assert verdict["spots"] == [[1, 2]]


def test_nearby_independent_changes_do_not_chain_into_one_operation():
    verdict = decide({"w-greedy": "a b c d e f g h i", "gigaam": "x b c y e f g z i"}, None, SPEECH)
...
def test_a_single_dissenting_decode_does_not_become_a_question():
    ...
    verdict = decide(
        {
            "w-greedy": "раз два три",
            "w-prompt": "раз два три",
            "w-fallback": "раз два три",
            "w-sample": "раз восемь три",
            "gigaam": "раз два четыре",
            "gigaam-ctc": "раз два четыре",
        },
        None,
        SPEECH,
    )
    options = [option["text"] for operation in verdict["review_operations"] for option in operation["alternatives"]]
...
def test_a_reading_the_majority_corrects_is_applied_without_being_asked():
    ...
    verdict = decide(
        {
            "w-greedy": "раз двое три альфа",
            "w-prompt": "раз два три альфа",
            "w-fallback": "раз два три альфа",
            "gigaam": "раз два три бета",
            "gigaam-ctc": "раз два три бета",
        },
        None,
        SPEECH,
    )
    settled = [operation for operation in verdict["review_operations"] if len(operation["alternatives"]) == 1]
...
def test_a_lone_gigaam_dissent_keeps_its_operation():
    ...
    verdict = decide(
        {
            "w-greedy": "прошло еще час либо два",
            "w-prompt": "прошло еще час либо два",
            "w-fallback": "прошло еще час либо два",
            "w-sample": "прошло еще час либо два",
            "fw-beam": "прошло еще час либо два",
            "gigaam": "прошло еще час или два",
...
def test_a_confirmed_term_removes_that_spot_everywhere():
    ...
    candidates = {
        "w-greedy": "открой issue сейчас",
        "w-prompt": "открой issue сейчас",
        "gigaam": "открой ишью сейчас",
        "gigaam-ctc": "открой ишью сейчас",
    }
...
def test_an_unconfirmed_cross_script_pair_is_still_a_question():
    ...
    verdict = decide(
        {
            "w-greedy": "перевод audi в текст",
            "w-prompt": "перевод audi в текст",
            "gigaam": "перевод аудио в текст",
            "gigaam-ctc": "перевод аудио в текст",
        },
        None,
        SPEECH,
    )
...
def test_number_notation_alone_is_not_an_operation():
    ...
    verdict = decide(
        {
            "w-greedy": "жди 10 секунд и пиши",
            "w-prompt": "жди 10 секунд и пиши",
            "gigaam": "жди десять секунд а пиши",
            "gigaam-ctc": "жди десять секунд а пиши",
        },
        None,
        SPEECH,
    )
...
def test_a_repeated_phrase_is_a_loop_too():
    ...
    verdict = decide(
        {"w-greedy": "спасибо большое " * 6, "gigaam": "спасибо большое " * 6},
        None,
        {"no_speech": 0.3, "peak_db": -20.0},
    )
    assert verdict["status"] == "review"
...
def test_a_short_genuine_repetition_is_not_a_loop():
    ...
    assert decide({"w-greedy": "да да да", "gigaam": "да да да"}, None, SPEECH)["status"] == "accepted"
...
def test_how_a_number_is_written_is_not_a_disagreement():
    ...
    verdict = decide({"w-greedy": "последних 24 часа", "gigaam": "последних двадцать четыре часа"}, None, SPEECH)
    assert verdict["status"] == "accepted"


def test_a_different_number_still_is_one():
    # "5" against "6" is a question about what was said, and stays a question.
    assert decide({"w-greedy": "нужно 5 штук", "gigaam": "нужно 6 штук"}, None, SPEECH)["status"] == "review"
...
def test_separate_digits_are_not_summed_into_agreement():
    ...
    assert decide({"w-greedy": "возьми 2 3", "gigaam": "возьми два три"}, None, SPEECH)["status"] == "review"
