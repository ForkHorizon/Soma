...
from asr_eval import (   # noqa: E402
    contains, load_decodes, load_gold, paired_verdict, report, score, sign_test_p, spot_answers, spot_score,
)
...
VERDICTS = [{"file": "a.wav", "review_operations": [
...
def test_spot_answers_recovers_what_each_decision_rejected():
    progress = [{"file": "a.wav", "signature": "abc", "text": "push"},
...
def test_score_separates_a_config_that_ran_on_fewer_files():
    ...
    decodes = {"a.wav": {"full": "привет мир", "partial": "привет мир"},
               "b.wav": {"full": "второй файл"}}
...
def test_score_reports_latin_and_punctuation_reach():
    ...
    decodes = {"a.wav": {"eng": "сделай push,", "rus": "сделай пуш"},
               "b.wav": {"eng": "готово", "rus": "готово"}}
...
def test_score_keeps_per_file_wer_and_drift_for_paired_comparison():
    ...
    decodes = {"a.wav": {"cfg": "привет мир", "gigaam": "привет мир"},
...
def test_paired_verdict_counts_wins_losses_and_excludes_ties_and_gaps():
    ...
    now = {"a.wav": 0.05,     # win: dropped
           "b.wav": 0.20,     # tie: excluded
           "c.wav": 0.40}     # loss: rose
...
def test_load_decodes_merges_multiple_files(tmp_path):
    ...
    experiment.write_text('{"config": "w-p-p3-v1", "file": "a.wav", "text": "привет мир", "error": null}\n',
                          encoding="utf-8")
    main_cache = tmp_path / "decodes.jsonl"
    main_cache.write_text('{"config": "gigaam", "file": "a.wav", "text": "привет мир", "error": null}\n',
                          encoding="utf-8")
...
def test_load_gold_excludes_consensus_rows_unless_explicitly_requested(tmp_path):
    ...
    path.write_text('{"file":"human.wav","text":"да","source":"review-session"}\n'
...
def test_report_baseline_config_compares_every_result_against_one_fixed_reference(capsys):
    ...
    decodes = {"a.wav": {"w-p-p3-v1": "привет мир", "gigaam": "привет мир"},
