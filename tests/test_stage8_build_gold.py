...
def test_accept_tier1_prod_pair_plus_all_confirm(tmp_path):
    ...
    exp.mkdir()
    texts = {"w-greedy": "Привет, как дела?", "gigaam": "привет как дела",
             "parakeet": "Привет, как дела?", "rnnt": "привет как дела"}
    _write_voices(exp, {k: [{"event": "decode", "file": "a.wav", "config": k,
                             "text": t, "error": None}]
    ...
    rc = s8.build(out, exp)
    assert rc == 0
    gold = [json.loads(l) for l in open(out / "gold-stage8-auto.jsonl")]
    queue = [json.loads(l) for l in open(out / "review-queue-stage8.jsonl")]
...
def test_prod_pair_split_goes_to_hard_review(tmp_path):
    exp = tmp_path / "experiments"
    exp.mkdir()
    texts = {"w-greedy": "открыть settings", "gigaam": "открыть сетингс",
             "parakeet": "открыть settings", "rnnt": "открыть сетингс"}
    _write_voices(exp, {k: [{"event": "decode", "file": "a.wav", "config": k,
                             "text": t, "error": None}]
                        for k, t in texts.items()})
    out = tmp_path / "out"
...
def test_error_rows_never_count_as_voices(tmp_path):
    exp = tmp_path / "experiments"
    exp.mkdir()
    rows = [{"event": "decode", "file": "a.wav", "config": k,
             "text": "привет", "error": None} for k in ("w-greedy", "gigaam")]
    rows.append({"event": "decode", "file": "a.wav", "config": "parakeet",
                 "text": "", "error": "RuntimeError"})
    rows.append({"event": "decode", "file": "a.wav", "config": "rnnt",
                 "text": "", "error": "RuntimeError"})
    _write_voices(exp, {"w-greedy": rows[:1], "gigaam": rows[1:2],
                        "parakeet": rows[2:3], "rnnt": rows[3:4]})
    out = tmp_path / "out"
    out.mkdir()
    s8.build(out, exp)
    gold = [json.loads(l) for l in open(out / "gold-stage8-auto.jsonl")]
...
def test_three_majority_without_prod_pair_is_hard_not_accepted(tmp_path):
    ...
    exp = tmp_path / "experiments"
    exp.mkdir()
    texts = {"w-greedy": "запусти тесты", "gigaam": "запусти тест",
             "parakeet": "запусти тесты", "rnnt": "запусти тесты"}
    _write_voices(exp, {k: [{"event": "decode", "file": "a.wav", "config": k,
                             "text": t, "error": None}]
                        for k, t in texts.items()})
    out = tmp_path / "out"
...
def test_qwen_voice_optional_and_used_as_confirmand(tmp_path):
    exp = tmp_path / "experiments"
    exp.mkdir()
    texts = {"w-greedy": "привет", "gigaam": "привет",
             "parakeet": "пока", "rnnt": "пока",
             "qwen3": "привет"}
    _write_voices(exp, {k: [{"event": "decode", "file": "a.wav", "config": k,
                             "text": t, "error": None}]
                        for k, t in texts.items()})
...
def test_qwen_preflight_omission_becomes_explicit_empty_not_disappearance(tmp_path):
    ...
    rows = {k: [{"event": "decode", "file": "zero.wav", "config": k,
                "text": "", "error": None}]
