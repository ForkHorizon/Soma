...
async def main():
    ...
    if not resp or resp.get("id") != 1:
        ...
        sys.exit(1)
    print(f"<- initialize OK. Server info: {resp['result']['serverInfo']['name']} v{resp['result']['serverInfo']['version']}")
    ...
    expected_tools = {
        "soma_prepare_context", "soma_get_map", "soma_ask", "soma_code_context", 
        "soma_scene", "soma_inspect", "soma_debug", "soma_review", "soma_delta", 
        "soma_apply", "soma_execute", "soma_remember"
    }
    ...
    await send_request("tools/call", {
        "name": "soma_ask",
        "arguments": {"question": "What tools are available?"}
    }, id=4)
    resp = await read_response()
