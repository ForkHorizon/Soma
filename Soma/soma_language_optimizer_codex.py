...
def _codex_translate_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "type": "object", "additionalProperties": False, "properties": {"status": {"type": "string", "enum": ["ok", "failed"]}, "source_language": {"type": "string"}, "translation_status": {"type": "string", "enum": ["translated", "original_english", "failed"]}, "translation": {"type": "string"}, "warnings": _schema_string_list()}, "required": ["status", "source_language", "translation_status", "translation", "warnings"]}


def _codex_improve_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "type": "object", "additionalProperties": False, "properties": {"status": {"type": "string", "enum": ["ok", "failed"]}, "improved_prompt": {"type": "string"}, "warnings": _schema_string_list()}, "required": ["status", "improved_prompt", "warnings"]}


def _run_codex_json(*, prompt: str, schema: dict[str, Any], model: str, timeout: float, temp_prefix: str, reasoning_effort: str | None = None) -> tuple[dict[str, Any] | None, dict[str, Any]]:
...
def _codex_command(model, schema_path, output_path, reasoning_effort):
    ...
    return [os.environ.get("SOMA_CODEX_BIN", "codex"), "exec", "--model", model, "-c", f'model_reasoning_effort="{effort}"', "--sandbox", "read-only", "--cd", str(root), "--ephemeral", "--ignore-rules", "--color", "never", "--output-schema", str(schema_path), "--output-last-message", str(output_path), "-"]


def _run_codex_command(cmd, prompt, timeout, environment, started, model):
    try:
        return subprocess.run(cmd, input=prompt, text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=timeout, env=environment, check=False)
...
def _decode_codex_result(completed, output_path, started, model):
    ...
    if completed.returncode != 0:
        return None, {"provider": "codex", "model": model, "status": "failed", "error": _clip_text((completed.stderr or completed.stdout or "").strip(), 2000), "seconds": time.monotonic() - started}
    if not isinstance(decoded, dict):
        return None, {"provider": "codex", "model": model, "status": "failed", "error": "Codex returned invalid JSON.", "raw": _clip_text(response_text or "", 2000), "seconds": time.monotonic() - started}
...
def _translate_general_prompt_codex(original: str, source_language: str, translator_model: str, model_profile: str, timeout: float) -> dict[str, Any]:
    ...
    decoded, meta = _run_codex_json(prompt=_codex_translation_prompt(source_language, protected), schema=_codex_translate_schema(), model=translator_model, timeout=timeout, temp_prefix="soma-rus-prompt-codex-translate-")
    if not isinstance(decoded, dict):
...
def _improve_general_prompt_codex(translation: str, improver_model: str, model_profile: str, timeout: float) -> dict[str, Any]:
    ...
    decoded, meta = _run_codex_json(prompt=_codex_improvement_prompt(protected), schema=_codex_improve_schema(), model=improver_model, timeout=timeout, temp_prefix="soma-rus-prompt-codex-improve-")
    if not isinstance(decoded, dict):
