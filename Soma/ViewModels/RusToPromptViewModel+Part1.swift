...
extension RusToPromptViewModel {
    static let translatorPresets: [RusToPromptModelPreset] = [
        RusToPromptModelPreset(model: "deepseek-v4-pro", quality: "Best", speed: "Fast", ram: "Paid API", detail: "Recommended translator (bench winner): best quality (0.97), fastest (~2.8s), zero placeholder/empty defects, ~0 local RAM. Token cost is sub-cent at personal volume.", recommended: true, provider: "deepseek"),
        RusToPromptModelPreset(model: "deepseek-v4-flash", quality: "High", speed: "Fast", ram: "Paid API", detail: "Cheaper DeepSeek translator. Near-Pro quality (0.94) at lower token cost; good default for bulk.", recommended: false, provider: "deepseek"),
        RusToPromptModelPreset(model: "gpt-5.4-mini", quality: "Best", speed: "Medium", ram: "0 GB", detail: "Codex translator via subscription: strong cloud option, no local RAM use.", recommended: false, isCodex: true),
        ...
        RusToPromptModelPreset(model: "gemma4:e4b-mlx", quality: "High", speed: "Balanced", ram: "9.6 GB", detail: "Best local translator option (0.95, clean output). Use for offline / privacy; costs local RAM.", recommended: false),
        RusToPromptModelPreset(model: "qwen3.5:9b", quality: "Good", speed: "Balanced", ram: "6.6 GB", detail: "Lighter local fallback; mid quality (0.57 bench), occasional placeholder leaks.", recommended: false),
        RusToPromptModelPreset(model: "qwen3:8b", quality: "Good", speed: "Fast", ram: "5.2 GB", detail: "Fast local translation with lower memory pressure.", recommended: false),
    ]
    static let analyzerPresets: [RusToPromptModelPreset] = [
        RusToPromptModelPreset(model: "deepseek-v4-pro", quality: "Best", speed: "Fast", ram: "Paid API", detail: "Recommended improver: top prompt-polish quality at ~0 local RAM. Keeps the daily flow online and RAM-free.", recommended: true, provider: "deepseek"),
        RusToPromptModelPreset(model: "deepseek-v4-flash", quality: "High", speed: "Fast", ram: "Paid API", detail: "Cheaper DeepSeek improver. Lower token cost for prompt polishing.", recommended: false, provider: "deepseek"),
        RusToPromptModelPreset(model: "gpt-5.4-mini", quality: "Best", speed: "Medium", ram: "0 GB", detail: "Codex improver via subscription: strong prompt-polish quality, no local RAM use.", recommended: false, isCodex: true),
        ...
        RusToPromptModelPreset(model: "gemma4:e4b-mlx", quality: "High", speed: "Balanced", ram: "9.6 GB", detail: "Best local improver if you want it offline. Costs local RAM — prefer DeepSeek on a memory-tight machine.", recommended: false),
        RusToPromptModelPreset(model: "qwen3:14b", quality: "High", speed: "Medium", ram: "9.3 GB", detail: "Balanced local analyzer when you want a mid-size model.", recommended: false),
    ]
    static let confidencePresets: [RusToPromptModelPreset] = [
        RusToPromptModelPreset(model: "gpt-5.4-mini", quality: "Best", speed: "Medium", ram: "0 GB", detail: "Strict confidence judge via Codex CLI. Strong at detecting invented requirements, meta-prompts, and lost technical spans; use when you want a tougher review.", recommended: false),
        ...
        RusToPromptModelPreset(model: "deepseek-v4-flash", quality: "High", speed: "Fast", ram: "Paid API", detail: "DeepSeek paid API confidence judge. Lower-cost default DeepSeek option for quality checks.", recommended: false, provider: "deepseek"),
        RusToPromptModelPreset(model: "deepseek-v4-pro", quality: "Best", speed: "Medium", ram: "Paid API", detail: "DeepSeek paid API confidence judge. Use for stricter quality checks when token cost is acceptable.", recommended: false, provider: "deepseek"),
        ...
        RusToPromptModelPreset(model: "gemini-3.1-flash-lite-preview", quality: "Good", speed: "Fast", ram: "0 GB", detail: "Recommended judge for daily use: fast, cheap/free-tier-friendly, 0 RAM, good quality (0.92 bench).", recommended: true, provider: "gemini"),
    ...
    func transform(mode: RusToPromptMode = .fullPrompt, somaViewModel: SomaViewModel, ollama: OllamaManager, queueManager: RusToPromptQueueManager? = nil) {
    ...
    func runTransform(prompt: String, mode: RusToPromptMode = .fullPrompt, somaViewModel: SomaViewModel, ollama: OllamaManager, queueManager: RusToPromptQueueManager?) async {
        do {
            ...
            if mode == .translateOnly {
                await MainActor.run { phase = .done; ollama.checkStatus() }
                return
            }
