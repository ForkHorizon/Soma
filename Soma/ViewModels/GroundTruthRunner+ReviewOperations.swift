...
extension GroundTruthRunner {
    ...
    nonisolated static func operationQueue(_ verdicts: [GroundTruthVerdict],
                                           settled: Set<String> = [],
                                           decided: [String: [String: String]] = [:]) -> [GroundTruthReviewItem] {
        ...
        pending.sort { $0.operations.count == $1.operations.count ? captureTime($0.verdict.file) > captureTime($1.verdict.file) : $0.operations.count < $1.operations.count }
        ...
        return (cheap.indices.flatMap { [cheap[$0], hard[$0]] } + hard.dropFirst(cheap.count))
            .flatMap { verdict, operations in
                operations.enumerated().map { GroundTruthReviewItem(verdict: verdict, operation: $0.element, index: $0.offset + 1, total: operations.count) }
            }
    }
    ...
    nonisolated static func needsAHuman(for verdict: GroundTruthVerdict,
                                        decided: [String: String] = [:]) -> [GroundTruthReviewOperation] {
    ...
    nonisolated static func captureTime(_ file: String) -> Int64 { Int64(String(file.drop(while: { !$0.isNumber }).prefix(while: { $0.isNumber }))) ?? .min }
    nonisolated static func fallbackOperation(for verdict: GroundTruthVerdict) -> GroundTruthReviewOperation {
        GroundTruthReviewOperation(id: "whole-recording", signature: "whole-recording", anchor: 0..<(verdict.candidates["w-greedy"]?.split(separator: " ").count ?? 0), seconds: nil, contextBefore: "", contextAfter: "", alternatives: Dictionary(grouping: verdict.candidates, by: { $0.value }).map { GroundTruthOperationAlternative(names: $0.value.map(\.key).sorted(), text: $0.key) })
    }
    ...
    static func operations(_ raw: Any?) -> [GroundTruthReviewOperation] {
        (raw as? [[String: Any]] ?? []).compactMap { row -> GroundTruthReviewOperation? in
            guard let id = row["id"] as? String, let signature = row["signature"] as? String, let anchor = row["anchor"] as? [Int], anchor.count == 2 else { return nil }
            let alternatives = (row["alternatives"] as? [[String: Any]] ?? []).compactMap { option -> GroundTruthOperationAlternative? in guard let names = option["names"] as? [String], let text = option["text"] as? String else { return nil }; return GroundTruthOperationAlternative(names: names, text: text) }
            let seconds = (row["seconds"] as? [Double]).flatMap { $0.count == 2 && $0[1] > $0[0] ? $0[0]...$0[1] : nil }
            let context = row["context"] as? [String] ?? []
            return GroundTruthReviewOperation(id: id, signature: signature, anchor: anchor[0]..<anchor[1], seconds: seconds, contextBefore: context.first ?? "", contextAfter: context.count > 1 ? context[1] : "", alternatives: alternatives)
        }
    }
}
