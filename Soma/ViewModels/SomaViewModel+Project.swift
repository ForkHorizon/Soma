...
extension SomaViewModel {
    ...
    func hydrateProjectRootsIfNeeded() {
        ...
        var targetRoot = ""
        if !storedLastProjectRoot.isEmpty {
            targetRoot = storedLastProjectRoot
        } else if let env = envRoot, !env.isEmpty {
            targetRoot = env
        } else if cwd != "/" && cwd != "/Users/\(NSUserName())" {
            targetRoot = cwd
        }
    ...
    func decodeStringMap(_ json: String) -> [String: String] {
        guard let data = json.data(using: .utf8), let decoded = try? JSONDecoder().decode([String: String].self, from: data) else {
            return [:]
        }
        return decoded
    }
    func decodeIntMap(_ json: String) -> [String: Int] {
        guard let data = json.data(using: .utf8), let decoded = try? JSONDecoder().decode([String: Int].self, from: data) else {
            return [:]
        }
        return decoded
    }
