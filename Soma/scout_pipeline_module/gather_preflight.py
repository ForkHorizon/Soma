...
def build_preflight(
    prompt, project_root, project_type, discovered, repo_index, git_status, git_diff_summary, collection_plan=None
):
    ...
    error_paths, log_candidates = _log_candidates(
        prompt,
        project_root,
        discovered,
        prompt_terms,
...
def _changed_paths(project_root, git_diff_summary, is_noise_path, normalize_path):
    ...
    changed.update(
        normalize_path(os.path.join(project_root, path))
...
def _log_candidates(
    ...
    for item in discovered:
        ...
        error_paths, candidates = _append_log_candidate(
            item, errors, error_paths, candidates, group_compile_errors, normalize_path
        )
