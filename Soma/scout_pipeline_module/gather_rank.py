...
def file_rank(item, terms, intent, project_type, packet_mode='debug', changed_paths=None, explicit_paths=None, error_paths=None, project_root=None, focus_root=None, open_source_review=False, collection_plan=None):
    ...
    context = _rank_context(item, terms, intent, packet_mode, changed_paths, explicit_paths, error_paths, project_root, collection_plan, split_identifier_terms, normalize_path)
    ...
    if generated_dependency and context['normalized'] not in context['explicit_paths'] and item['path'] not in context['explicit_paths']:
        score -= 70
    return score + min(int(max(0, item['mtime']) / 10000000), 15)


def _rank_context(item, terms, intent, packet_mode, changed_paths, explicit_paths, error_paths, project_root, collection_plan, split_identifier_terms, normalize_path):
...
def _direct_match_score(ctx):
    ...
    if ctx['rel'] in ctx['changed_paths'] or item['path'] in ctx['changed_paths'] or ctx['normalized'] in ctx['changed_paths']:
...
def _category_score(ctx, project_type, packet_mode):
    ...
    score += 45 if category == 'script' and ('script' in ctx['terms'] or 'script' in ctx['intent']['reason'].lower()) else (25 if category == 'script' else 0)
    score += 45 if category == 'source' and packet_mode in {'changes', 'review', 'implementation'} else (25 if category == 'source' else 0)
    return score
...
def _project_type_score(ctx, project_type, project_owned, generated_dependency):
    ...
    score = 25 if ctx['lowered_name'] in names or (project_type == 'swift' and ctx['lowered_name'].endswith('.xcodeproj')) else 0
    score += suffix_score if ctx['item']['path'].endswith(suffixes) else 0
    score -= 90 if project_type == 'swift' and ctx['category'] == 'source' and not ctx['item']['path'].endswith('.swift') else 0
    return score


def _unity_score(ctx, project_owned, generated_dependency):
    score = 55 if project_owned else 0
    if generated_dependency and ctx['normalized'] not in ctx['explicit_paths'] and ctx['item']['path'] not in ctx['explicit_paths']:
        score -= 140
...
def _android_icon_score(ctx, project_type):
    if project_type != 'unity' or not ((ctx['term_set'] & {'apk', 'android'}) and (ctx['term_set'] & {'icon', 'icons', 'launcher', 'mipmap', 'adaptive'})):
        return 0
    ...
    score += 220 if rel_lower.startswith('assets/') and 'icon' in rel_lower and rel_lower.endswith(('.png.meta', '.png', '.asset', '.meta')) else 0
    score += 80 if rel_lower == 'projectsettings/androidresolverdependencies.xml' else 0
    return score


def _open_source_score(ctx):
    release_doc_names = {'package.json', 'readme.md', 'license.md', 'license', 'changelog.md', 'documentation.md', 'api_reference.md'}
    rel_lower = ctx['rel_lower']
    ...
    score += 110 if name in {'mcpserver.cs', 'mcpservermethods.cs', 'nexus_unity_bridge.py', 'schemas.py', 'routing.py', 'client.py'} else 0
    score -= 260 if rel_lower.endswith(('.unity', 'autosavedscene.unity')) else 0
    score -= 240 if rel_lower.startswith('projectsettings/') or name in {'unityconnectsettings.asset', 'scenetemplatesettings.json'} else 0
    return score
...
def _plan_required_score(ctx):
    ...
    score += 140 if 'core_entrypoints' in required and ctx['lowered_name'] in {'mcpserver.cs', 'mcpservermethods.cs', 'nexus_unity_bridge.py', 'main.py', 'server.py'} else 0
    return score
...
def _local_ai_score(ctx):
    if not ('ollama' in ctx['term_set'] or ('local' in ctx['term_set'] and {'ai', 'model'} & ctx['term_set'])):
        return 0
    haystack = ' '.join([ctx['lowered_name'], ctx['rel_lower'], ctx['lowered_path'], ctx['symbol_text'], ' '.join(ctx['search_terms'])])
    ...
    score += 160 if ctx['term_set'] & {'configurable', 'settings', 'state', 'set', 'interval', 'time', 'application'} and ctx['lowered_name'] in {'globalsettingsbar.swift', 'somaviewmodel.swift'} else 0
    signals = ('ismodelloaded', 'isollamarunning', 'startmodel', 'start model', 'launchollama', 'launch ollama', 'ollamaaction', 'ollama action', 'sendkeepalive', 'keep_alive')
...
def _quiet_hours_score(ctx):
    if not ('quiet' in ctx['term_set'] and {'hours', 'hour', 'midnight'} & ctx['term_set']):
        return 0
    names = {'appstate.swift', 'cooldownpolicy.swift', 'nudgescheduler.swift', 'moodlingsettings.swift', 'settingsview.swift', 'cooldownpolicytests.swift'}
    ...
    score += 35 if any(term in ctx['rel_lower'] for term in ('cooldown', 'nudge', 'scheduler', 'settings', 'appstate')) else 0
    return score
