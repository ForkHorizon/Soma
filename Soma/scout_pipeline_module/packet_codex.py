...
def _codex_parts(user_prompt, bundle, token_budget, max_tokens, evidence_items, preview_chars):
    ...
    _extend_named(parts, 'Normalized errors:', [f'- {line}' for line in (bundle.get('error_lines') or [])[:MAX_ERROR_LINES]])
...
def _append_language_optimization(parts, bundle):
    ...
    if source_language and source_language != 'en':
        parts.extend([f"- Original language: {source_language}", f"- Language optimization: {info.get('status')}", "- Expected answer language: English"])
...
def _append_graph_suggestions(parts, bundle, evidence_items):
    ...
    files = [str(path).strip() for path in ((bundle.get('omitted_context') or {}).get('graph_suggested_files') or []) if str(path).strip()]
    if not (suggestions or files):
        return
    parts.extend(['', 'Graph suggested:'])
    if files:
        selected_total = len(evidence_items)
        parts.append(f"- Files {min(len(files), selected_total)}/{selected_total}: graph query/affected hints boosted project-local evidence only.")
...
def _append_evidence(parts, user_prompt, evidence_items, preview_chars):
    ...
    for index, item in enumerate(evidence_items, start=1):
        source_preview_count = _append_evidence_item(parts, user_prompt, item, index, preview_chars, source_preview_count, format_line_range)
