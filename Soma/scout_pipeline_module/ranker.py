...
def fallback_summary(prompt, project_root, project_type, evidence_items, error_lines, packet_mode='debug'):
    ...
    wants_log_evidence = packet_mode == 'debug' or any(term in lowered_prompt for term in ('log', 'logs', 'error', 'traceback', 'crash', 'exception', 'fail', 'failure'))
...
async def summarize_with_ollama(prompt, project_root, project_type, evidence_items, error_lines):
    ...
    response = (await query_ollama([{'role': 'system', 'content': OLLAMA_SUMMARY_SYSTEM}, {'role': 'user', 'content': json.dumps(summary_payload)}], timeout=OLLAMA_SUMMARY_TIMEOUT, stage='summary'))
...
def pinned_evidence_ids(prompt, preflight, evidence_items):
    ...
    android_icon = (
        bool(terms & {'apk', 'android'})
        and bool(terms & {'icon', 'icons', 'launcher', 'mipmap', 'adaptive'})
    )
    ...
    for index, item in enumerate(evidence_items):
        ...
        if path.endswith('/ollamamanager.swift'):
            pinned.append(index)
        if configurable and (
            path.endswith('/globalsettingsbar.swift')
            or path.endswith('/somaviewmodel.swift')
        ):
            pinned.append(index)
    return list(dict.fromkeys(pinned))
...
async def filter_candidates_with_model(prompt, preflight, evidence_items, max_items=MAX_EVIDENCE_ITEMS):
    ...
    if len(evidence_items) <= max_items:
        return (evidence_items, {'stage': 'candidate_filter', 'model': RANKER_MODEL, 'status': 'skipped', 'candidate_count_before': len(evidence_items), 'candidate_count_after': len(evidence_items)})
    ...
    response = (await query_ollama_model(RANKER_MODEL, [
        {'role': 'system', 'content': 'Choose the best evidence candidates for a compact code packet. Return JSON only: {"selected_ids":[0,1],"notes":["..."]}. Use only candidate ids.'},
        {'role': 'user', 'content': user_payload},
    ], timeout=30, num_predict=220, json_mode=True, stage='candidate_filter', metadata={'candidate_count_before': len(evidence_items)}))
    base_stage = {
    ...
    if ('error' in response):
        return (evidence_items[:max_items], {**base_stage, 'status': 'failed', 'error': response['error'], 'candidate_count_after': max_items, 'candidate_tokens_after': _estimate_stage_tokens(json.dumps(ranker_payload(prompt, preflight, evidence_items[:max_items])))})
    ...
    if not isinstance(decoded, dict) or not isinstance(decoded.get('selected_ids'), list):
        return (evidence_items[:max_items], {**base_stage, 'status': 'failed', 'error': 'invalid candidate_filter JSON', 'candidate_count_after': max_items, 'candidate_tokens_after': _estimate_stage_tokens(json.dumps(ranker_payload(prompt, preflight, evidence_items[:max_items])))})
    selected = []
    seen = set()
    ...
    return (selected, {**base_stage, 'status': 'ok', 'notes': _string_list(decoded.get('notes')), 'candidate_count_after': len(selected), 'candidate_tokens_after': after_tokens, 'local_ai_net_savings_tokens': max(0, base_stage['candidate_tokens_before'] - after_tokens)})
...
async def rank_evidence_with_model(prompt, preflight, evidence_items):
    ...
    for (attempt, system) in enumerate(prompts, start=1):
        start = time.monotonic()
        response = (await query_ollama_model(RANKER_MODEL, [{'role': 'system', 'content': system}, {'role': 'user', 'content': payload}], timeout=25, num_predict=(180 if (attempt == 1) else 96), json_mode=True, stage='ranker', metadata={'candidate_count_before': len(evidence_items)}))
        duration_ms = (time.monotonic() - start) * 1000
        if ('error' in response):
            return (evidence_items, {'stage': 'ranker', 'model': RANKER_MODEL, 'status': 'failed', 'error': response['error'], 'duration_ms': duration_ms})
        decoded = extract_json_object(response.get('message', {}).get('content', ''))
    ...
    return (ordered, {'stage': 'ranker', 'model': RANKER_MODEL, 'status': 'ok', 'notes': _string_list(decoded.get('notes')), 'candidate_count_before': len(evidence_items), 'candidate_count_after': len(ordered), 'duration_ms': duration_ms if 'duration_ms' in locals() else 0})
...
async def analyze_packet_with_model(prompt, preflight, evidence_items, error_lines):
    ...
    for (attempt, system) in enumerate(prompts, start=1):
        start = time.monotonic()
        response = (await query_ollama_model(ANALYST_MODEL, [{'role': 'system', 'content': system}, {'role': 'user', 'content': user_payload}], timeout=45, num_predict=280, json_mode=True, stage='analyst', metadata={'candidate_count_before': len(evidence_items)}))
        duration_ms = (time.monotonic() - start) * 1000
        if ('error' in response):
            return (None, {'stage': 'analyst', 'model': ANALYST_MODEL, 'status': 'failed', 'error': response['error'], 'duration_ms': duration_ms})
        decoded = extract_json_object(response.get('message', {}).get('content', ''))
    ...
    return (decoded, {'stage': 'analyst', 'model': ANALYST_MODEL, 'status': 'ok', 'duration_ms': duration_ms if 'duration_ms' in locals() else 0})
...
async def referee_evidence_with_model(prompt, preflight, evidence_items, evidence_quality):
    ...
    payload = {
        ...
        'evidence': [
            {'id': index, 'path': item.get('path'), 'kind': item.get('kind'), 'reason': item.get('reason'), 'symbols': (item.get('symbols') or [])[:10]}
            for (index, item) in enumerate(evidence_items)
        ],
    }
    user_payload = json.dumps(payload)
    start = time.monotonic()
    response = (await query_ollama_model(RANKER_MODEL, [
        {'role': 'system', 'content': 'Review selected evidence quality for a code packet. Return JSON only: {"status":"ok|degraded","warnings":["..."],"missing_context":["..."]}. Do not invent files.'},
        {'role': 'user', 'content': user_payload},
    ], timeout=25, num_predict=180, json_mode=True, stage='quality_referee', metadata={'candidate_count_before': len(evidence_items), 'candidate_count_after': len(evidence_items)}))
    stage = {'stage': 'quality_referee', 'model': RANKER_MODEL, 'candidate_count_before': len(evidence_items), 'candidate_count_after': len(evidence_items), 'candidate_tokens_before': _estimate_stage_tokens(user_payload), 'candidate_tokens_after': _estimate_stage_tokens(user_payload), 'duration_ms': (time.monotonic() - start) * 1000}
    if ('error' in response):
    ...
    return (updated, {**stage, 'status': 'ok', 'referee_status': decoded.get('status') or updated.get('status'), 'notes': updated['warnings'][:4]})
