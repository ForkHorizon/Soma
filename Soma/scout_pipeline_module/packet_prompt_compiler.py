...
def _append_focus(parts, bundle):
    ...
    if not preflight.get("focus_root"):
        return
    parts.extend(
        [f"- Focus: {preflight.get('focus_root')}", f"- Focus type: {preflight.get('focus_kind') or 'package'}"]
    )
...
def _open_source_evidence(evidence_items, preflight):
    ...
    for item in evidence_items:
        ...
        if path_lower.endswith(
            (".unity", "/projectsettings.asset", "/unityconnectsettings.asset", "/scenetemplatesettings.json")
        ):
            continue
...
def _android_icon_evidence(evidence_items, user_prompt):
    ...
    if not (("apk" in lowered or "android" in lowered) and ("icon" in lowered or "icons" in lowered)):
        return []
    return [
        item for item in evidence_items if _android_icon_path(str(item.get("path") or "").replace("\\", "/").lower())
    ]
