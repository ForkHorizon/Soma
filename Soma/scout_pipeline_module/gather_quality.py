...
def _evidence_satisfies_requirement(item, requirement):
    ...
    if requirement == "core_entrypoints":
        return (
            name in {"mcpserver.cs", "mcpservermethods.cs", "nexus_unity_bridge.py", "server.py", "main.py"}
            or "/runtime/" in path
        )
...
def assess_plan_alignment(collection_plan, evidence_items, preflight=None):
    ...
    excluded_selected = _excluded_selected(
        collection_plan, evidence_items, preflight, is_generated_dependency_path, normalize_path
    )
...
def _requirement_patterns():
    return {
        ...
        "graphify_integration": [
            "Soma/gateway/graphify_adapter.py",
            "Soma/gateway/graph_storage.py",
            "Soma/scout_pipeline_module/pipeline.py",
            "Soma/gateway/tools/context.py",
            "README.md",
            "docs/**/*.md",
        ],
