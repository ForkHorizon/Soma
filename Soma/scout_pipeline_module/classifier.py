...
def is_open_source_readiness_prompt(prompt):
    ...
    return bool(
        re.search(
            r"\b(open[- ]?source|opensource|public repo|public repository|release readiness|publish|pre[- ]?release|release)\b",
            lowered,
        )
        or any(
            marker in lowered for marker in ("выпустить", "релиз", "опенсорс", "open source", "слабые", "доработать")
        )
    )
...
def packet_mode_for_prompt(prompt):
    ...
    if re.search("\\b(read[- ]only|investigate|investigation|analyze|analysis|inspect|diagnose)\\b", lowered):
        if re.search(
            "\\b(debug|crash|error|exception|fail|failing|failure|log|traceback|not work|broken|diagnose|slow|latency)\\b",
            lowered,
        ):
            return "debug"
        return "review"
