...
def _query_graphify_context(goal, project_root, budget=1200):
    ...
    if not isinstance(result, dict):
        return {'graphs': [], 'answers': [], 'warnings': ['graphify unavailable: invalid response'], 'project_only': True}
    return {
...
def _graph_matches_collection_scope(graph_result, collection_plan=None, preflight=None):
    ...
    return _graph_scope_match(graphs, target_scope, preflight.get('focus_root'), graph_result, normalize_path, is_generated_dependency_path)
