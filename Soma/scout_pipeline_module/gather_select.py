...
def evidence_policy_summary(evidence_items, project_root, project_type):
    ...
    generated = [item.get('path') or '' for item in evidence_items if is_generated_dependency_path(item.get('path') or '', project_root)]
    owned = [item.get('path') or '' for item in evidence_items if is_project_owned_path(item.get('path') or '', project_root, project_type)]
    warnings = []
    if generated:
...
def _selection_context(project_root, prompt, project_type, repo_index, preflight, max_items):
    ...
    return {'project_root': project_root, 'project_type': project_type, 'prompt': prompt, 'terms': terms, 'preview_terms': preview_terms, 'packet_mode': packet_mode, 'collection_plan': collection_plan, 'open_source_review': open_source_review, 'limit': max_items or MAX_EVIDENCE_ITEMS, 'rank_args': rank_args}


def _preview_terms(prompt, terms, term_set, project_type, prompt_terms):
    preview_terms = prompt_terms(prompt)
    android_icon = project_type == 'unity' and (term_set & {'apk', 'android'}) and (term_set & {'icon', 'icons', 'launcher', 'mipmap', 'adaptive'})
    if android_icon:
        return ['m_icons', 'buildtarget: android', 'android:icon', 'launcher', 'mipmap', 'platformsettings', 'icon', 'icons', 'android'] + preview_terms
...
def _selection_candidates(project_root, repo_index):
    ...
    discovered = [
        {'path': item['path'], 'relative_path': rel_path(item['path'], project_root), 'name': os.path.basename(item['path']), 'category': item['category'], 'mtime': item.get('mtime', 0), 'symbols': item.get('symbols') or [], 'unity_refs': item.get('unity_refs') or [], 'search_terms': item.get('search_terms') or []}
...
def _discovered_item(path, relative_path, category):
    try:
        ...
        mtime = 0
    return {'path': path, 'relative_path': relative_path, 'name': os.path.basename(path), 'category': category, 'mtime': mtime, 'symbols': [], 'unity_refs': [], 'search_terms': []}
...
def _select_scored_evidence(scored, indexed_by_path, context, include_generated):
    ...
    for item in scored:
        if _skip_scored_item(item, context, include_generated, category_counts, seen_paths, excluded_markers, is_generated_dependency_path):
            continue
...
def _category_limits(context):
    wants_log = context['packet_mode'] == 'debug' or bool(set(context['terms']) & {'log', 'logs', 'error', 'errors', 'traceback', 'crash', 'exception', 'fail', 'failure'})
    limits = {'manifest': 2, 'log': 2 if context['packet_mode'] == 'debug' else (1 if wants_log else 0), 'script': 2, 'source': 6 if context['packet_mode'] in {'debug', 'review', 'implementation'} else 4, 'config': 2, 'unity': 3, 'notes': 2}
    if context['open_source_review']:
        limits.update({'manifest': 4, 'log': 0, 'script': 3, 'source': 4, 'config': 3, 'unity': 2, 'notes': 4})
    return limits


def _skip_scored_item(item, context, include_generated, category_counts, seen_paths, excluded_markers, is_generated_dependency_path):
    category_limits = _category_limits(context)
    if not include_generated and is_generated_dependency_path(item['path'], context['project_root']) and item['path'] not in context['rank_args']['explicit_paths']:
        return True
...
def _evidence_from_item(item, context, indexed_by_path):
    ...
    return evidence_item_from_path(item['path'], item['category'], reason, context['preview_terms'], indexed_by_path.get(item['path']))
...
def _is_graphify_update_review_prompt(prompt, terms):
    ...
    update_terms = {'version', 'versions', 'changelog', 'changelogs', 'latest', 'update', 'updates', 'release', 'releases', 'feature', 'features'}
...
def _command_evidence_item(command, preview, reason):
    return {'path': f'command: {command}', 'kind': 'command', 'reason': reason, 'preview': preview.strip()[:1800] or '[No output]', 'start_line': None, 'end_line': None, 'symbols': [], 'unity_refs': []}
...
def _graphify_command_evidence(terms):
    evidence = [
        _command_evidence_item('graphify --version', _run_command(['graphify', '--version'], timeout=5), 'Included because Graphify version/update tasks need command evidence, not fixture guesses.'),
        _command_evidence_item(f'{sys.executable} -m pip index versions graphifyy', '\n'.join(_run_command([sys.executable, '-m', 'pip', 'index', 'versions', 'graphifyy'], timeout=12).splitlines()[:4]), 'Included to compare the installed Graphify package with the latest package index version.'),
    ]
    if {'changelog', 'changelogs', 'release', 'releases', 'feature', 'features'} & set(terms or []):
        evidence.append(_command_evidence_item('missing: Graphify changelog/release notes', 'No repo-local Graphify changelog is available inside the selected Soma project. Ask for the upstream Graphify changelog/release notes or browse the official repository before making feature-integration claims.', 'Included as explicit missing-context guidance for Graphify changelog/release review.'))
    return evidence
