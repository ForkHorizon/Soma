...
def extract_symbols(path, text=None):
    ...
    if ext == ".go":
        ...
        patterns = [
            ...
            r"\b(?:fun|void|public|private|protected|static)\s+[A-Za-z_][A-Za-z0-9_<>,\[\]?]*\s+([A-Za-z_][A-Za-z0-9_]*)\s*\(",
        ]
    elif ext == ".php":
    ...
    try:
        try:
            ...
            if stat.st_size > (10 * 1024):
                rust_scanner_dir = os.path.join(
                    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "rust_scanner"
                )
                rust_scanner_path = os.path.join(rust_scanner_dir, "target", "release", "rust_scanner")
                ...
                res = subprocess.run(
                    [rust_scanner_path, "extract-unity-refs", path], capture_output=True, text=True, timeout=10
                )
                if res.returncode == 0:
