...
def deterministic_collection_plan(prompt, project_root, project_type, project_map=None):
    ...
    graphify_update_review = (
        'graphify' in lowered or 'граффити' in lowered or 'графити' in lowered
    ) and (
        terms & {'version', 'versions', 'changelog', 'changelogs', 'release', 'releases', 'latest', 'update', 'updates', 'features', 'feature'}
    ...
    if graphify_update_review:
        ...
        style = 'review_packet'
        warnings.append('Graphify update/version review needs command or changelog evidence; request it if local files are insufficient.')
    ...
    elif intent.get('packet_mode') == 'debug' or terms & {'error', 'errors', 'log', 'logs', 'traceback', 'crash', 'broken'}:
        task_type = 'debug'
    ...
    if project_map:
        for package in project_map.get('package_manifests') or []:
            haystack = ' '.join(str(package.get(key) or '') for key in ('root', 'name', 'displayName', 'description')).lower()
...
def normalize_collection_plan(raw_plan, fallback_plan=None):
    ...
    expected_style = str(raw_plan.get('expected_packet_style') or fallback_plan.get('expected_packet_style') or 'review_packet')
    plan = {
        ...
        'required_evidence': _string_list(raw_plan.get('required_evidence')) or _string_list(fallback_plan.get('required_evidence')),
        'excluded_context': _string_list(raw_plan.get('excluded_context')) or _string_list(fallback_plan.get('excluded_context')),
        'expected_packet_style': expected_style if expected_style in PACKET_STYLES else fallback_plan.get('expected_packet_style', 'review_packet'),
...
async def plan_collection_with_local_model(prompt, project_root, project_type, discovered=None, repo_index=None, planning_mode='auto'):
    ...
    if planning_mode == 'off':
        ...
        }, 'deterministic_fallback', fallback.get('warnings', [])
    ...
    if 'error' in response:
        ...
        return plan, {**base_stage, 'status': 'failed', 'source': 'deterministic_fallback', 'error': response['error'], 'notes': warnings}, 'deterministic_fallback', warnings
    ...
    if not isinstance(decoded, dict):
        ...
        plan = normalize_collection_plan(fallback, fallback)
        plan['warnings'] = list(dict.fromkeys(plan.get('warnings', []) + warnings))
        return plan, {**base_stage, 'status': 'failed', 'source': 'deterministic_fallback', 'error': 'invalid planner JSON', 'notes': warnings}, 'deterministic_fallback', warnings
    ...
    if plan.get('confidence', 0) < 0.35:
        ...
        plan = normalize_collection_plan(fallback, fallback)
        plan['warnings'] = list(dict.fromkeys(plan.get('warnings', []) + warnings))
        return plan, {**base_stage, 'status': 'degraded', 'source': 'deterministic_fallback', 'notes': warnings}, 'deterministic_fallback', warnings

    return plan, {**base_stage, 'status': 'ok', 'source': 'local_model', 'notes': plan.get('warnings', [])[:4]}, 'local_model', plan.get('warnings', [])
...
async def referee_evidence_with_plan_model(prompt, collection_plan, evidence_items, evidence_quality):
    ...
    return result, {**stage, 'status': 'ok', 'referee_status': result['status'], 'notes': (result['warnings'] + result['missing_evidence'])[:4]}
