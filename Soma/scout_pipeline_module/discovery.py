    ...
    for root, dirs, files in os.walk(project_root):
        ...
        for name in files:
            ...
            try:
                ...
                mtime_ns = 0
            items.append(
                {"path": path, "name": name, "category": category, "mtime": mtime, "size": size, "mtime_ns": mtime_ns}
            )
...
def detect_project_type(project_root):
    ...
    if os.path.exists(os.path.join(project_root, "Assets")) and os.path.exists(
        os.path.join(project_root, "ProjectSettings")
    ):
    ...
    if (
        os.path.exists(os.path.join(project_root, "pom.xml"))
        or os.path.exists(os.path.join(project_root, "build.gradle"))
        or os.path.exists(os.path.join(project_root, "build.gradle.kts"))
    ):
    ...
    if (
        ...
        or os.path.exists(os.path.join(project_root, "pnpm-lock.yaml"))
        or os.path.exists(os.path.join(project_root, "yarn.lock"))
    ):
...
def build_repo_index(project_root, discovered):
    ...
    for item in discovered:
        ...
        if (
            cached
            and (cached.get("cache_id") == cache_id)
            and ("search_terms" in cached)
            ...
            changed_count += 1
            text = ""
            if (os.path.splitext(path)[1].lower() in TEXT_EXTENSIONS) or (
                item["category"] in {"source", "script", "config", "manifest", "unity"}
            ):
                text = read_text_file(path)
            search_terms = []
            if text:
                search_terms = list(
                    dict.fromkeys(split_identifier_terms(path) + split_identifier_terms(text[:MAX_FILE_BYTES]))
                )[:300]
            indexed = {
                "cache_id": cache_id,
                "path": path,
