    ...
    var body: some View {
        DisclosureGroup("What gets run, and with which parameters", isExpanded: $expanded) {
            VStack(alignment: .leading, spacing: 14) {
                note(
                    "Audio never leaves this Mac. Each recording is read as mono 16 kHz float and handed to a local engine venv; the app itself sends nothing over the network."
                )
                ...
                note(
                    "Only w-greedy and GigaAM RNNT run on every file. The other six decodes are spent solely on recordings where those two disagree, which is what keeps a full corpus run to a night instead of a week."
                )
                note(
                    "Votes are grouped by what they share. Six Whisper readings are one acoustic model seen six ways, and two GigaAM heads are one encoder decoded two ways. Accepting needs agreement ACROSS the two families: a GigaAM head matching at least two Whisper decodes, or matching w-greedy outright on the first pass. All six Whisper decodes agreeing with each other counts for nothing on its own."
                )
                note(
                    "Review rows play only the seconds under dispute, located from the tier-one decode's word timestamps, so one wrong word in a two-minute recording no longer costs a full listen."
                )
                note(
                ...
                note(
                    "Never auto-accepted: a token repeated six times or more, which is what a Whisper hallucination loop looks like even when every config produces it."
                )
                note(
                    "A recording is filed as no speech only on evidence: Whisper's own no_speech_prob above 0.5 — measured at 0.85 on a hallucinated \"Спасибо\" against 0.02 on real speech — together with GigaAM independently hearing nothing. Audible audio that Whisper is merely unsure about goes to a human instead."
                )
                note(
                    "Confirming a term costs no model time to apply: every decode stays on disk, so Apply glossary just re-votes the cached results and the queue shrinks in seconds."
                )
            }
            .padding(.top, 10)
        }
