...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.rus_to_prompt_confidence:
        ...
        return 0
    raise SystemExit(
        "choose --rus-to-prompt, --rus-to-prompt-translate, --rus-to-prompt-improve, or --rus-to-prompt-confidence"
    )
...
def _parse_args(argv):
    ...
    parser.add_argument(
        "--rus-to-prompt", action="store_true", help="Translate and polish a general prompt without project context."
    )
    parser.add_argument(
        "--rus-to-prompt-translate", action="store_true", help="Translate a general prompt without project context."
    )
    parser.add_argument(
        "--rus-to-prompt-improve", action="store_true", help="Improve an English prompt without project context."
    )
    parser.add_argument(
        "--rus-to-prompt-confidence",
        action="store_true",
        help="Score final prompt quality with Codex CLI without project context.",
    )
...
def _confidence_payload(args):
    return _api().score_general_prompt_confidence(
