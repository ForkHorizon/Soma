...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--graph-storage-json", action="store_true", help="Print managed Graphify storage info and exit"
    )
    parser.add_argument(
        "--check-graphify-tool-json", action="store_true", help="Print installed/latest Graphify tool version info"
    )
    parser.add_argument(
        "--tool-status-json", nargs="?", const="all", default=None, help="Print managed extension tool version info"
    )
    parser.add_argument(
        "--project-overview-json", action="store_true", help="Print selected project overview for the Soma Projects UI"
    )
    parser.add_argument(
        "--update-tool",
        choices=["codebase-memory", "graphify", "ponytail", "projectmem"],
        default=None,
        help="Update one managed extension tool and verify clients",
    )
    parser.add_argument(
        "--setup-memory-tools",
        action="store_true",
        help="Install and initialize Codebase-Memory and projectmem for the selected project",
    )
    parser.add_argument(
        "--setup-project-tool",
        choices=["codebase-memory", "projectmem", "graphify"],
        default=None,
        help="Install or initialize one extension tool for the selected project",
    )
    parser.add_argument(
        "--scan-ai-clients-json", action="store_true", help="Scan known project/client AI config locations"
    )
    parser.add_argument("--verify-ai-clients-json", action="store_true", help="Verify known AI client configs")
    parser.add_argument(
        "--sync-ai-clients", action="store_true", help="Install/repair known AI client configs for Soma"
    )
    parser.add_argument(
        "--sync-project-clients",
        action="store_true",
        help="Repair only project-local AI client configs for the selected project",
    )
    parser.add_argument(
        "--recent-project-root", action="append", default=[], help="Additional project root to scan/verify"
    )
    parser.add_argument(
        "--migrate-graph", action="store_true", help="Copy a legacy graphify-out into Soma managed graph storage"
    )
    parser.add_argument(
        "--refresh-managed-graph",
        action="store_true",
        help="Refresh the selected project's managed Graphify graph without project-root output",
    )
    parser.add_argument(
        "--refresh-all-managed-graphs", action="store_true", help="Refresh all real indexed managed Graphify graphs"
    )
    parser.add_argument(
        "--full-graph-rebuild",
        action="store_true",
        help="Use graphify extract instead of AST-only update for graph refresh",
    )
    parser.add_argument("--force-graph-refresh", action="store_true", help="Pass --force to AST-only graph refresh")
    parser.add_argument(
        "--diagnose-graph-json", action="store_true", help="Run Graphify multigraph diagnostics for the selected graph"
    )
    parser.add_argument(
        "--check-graph-semantic-update-json",
        action="store_true",
        help="Check whether semantic graph refresh is pending",
    )
    parser.add_argument(
        "--graph-tree-json", action="store_true", help="Generate managed Graphify tree report and print its path"
    )
    parser.add_argument(
        "--graph-callflow-json",
        action="store_true",
        help="Generate managed Graphify callflow report and print its path",
    )
...
def _add_config_args(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        "--install-codex-config", action="store_true", help="Back up and install a Soma-only Codex MCP config"
    )
    parser.add_argument(
        "--rollback-codex-config", action="store_true", help="Restore Codex config from the newest Soma backup"
    )
    parser.add_argument(
        "--install-gemini-config", action="store_true", help="Back up and install a Soma-only Gemini MCP config"
    )
    parser.add_argument(
        "--install-hermes-config", action="store_true", help="Back up and install a Soma-only Hermes MCP config"
    )
    parser.add_argument(
        "--rollback-gemini-config", action="store_true", help="Restore Gemini config from the newest Soma backup"
    )
    parser.add_argument(
        "--verify-client-config",
        choices=["codex", "gemini", "hermes"],
        default=None,
        help="Verify a client config points to Soma only",
    )
...
def _add_project_setup_args(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        "--analyze-project-ai-setup",
        action="store_true",
        help="Analyze global/project AI configs and prompts for Soma-first routing",
    )
    parser.add_argument(
        "--harden-project-ai-setup",
        action="store_true",
        help="Back up and rewrite project AI setup for Soma-first routing",
    )
    parser.add_argument(
        "--rollback-project-ai-setup",
        action="store_true",
        help="Restore files changed by the latest project AI setup hardening",
    )
...
def _setup_graphify_project(project_root: str | None) -> dict[str, Any]:
    ...
    issues = (
        graph.get("issues")
        or graph.get("warnings")
        or ([] if graph.get("status") == "ok" else [graph.get("summary", "graphify_setup_failed")])
    )
    return {
        "status": "ok" if graph.get("status") == "ok" and not issues else "degraded",
        "summary": "Graphify graph is ready for the selected project."
        if graph.get("status") == "ok" and not issues
        else graph.get("summary", "Graphify setup finished with issues."),
        "tool_id": "graphify",
        "name": "Graphify",
...
def _handle_graph_commands(args: argparse.Namespace, project_root: str | None) -> bool:
    commands = [
        ...
        (
            args.refresh_managed_graph,
            "graph refresh",
            lambda: graphify.storage.refresh_managed_graph(
                project_root, full=args.full_graph_rebuild, force=args.force_graph_refresh
            ),
        ),
        (args.diagnose_graph_json, "graph diagnostics", lambda: graphify.storage.diagnose_graph(project_root)),
        (
            args.check_graph_semantic_update_json,
            "graph semantic update check",
            lambda: graphify.storage.check_semantic_update(project_root),
        ),
        (args.graph_tree_json, "graph tree generation", lambda: graphify.storage.generate_tree_report(project_root)),
        (
            args.graph_callflow_json,
            "graph callflow generation",
            lambda: graphify.storage.generate_callflow_report(project_root),
        ),
    ]
...
def _handle_project_setup_commands(args: argparse.Namespace, project_root: str | None) -> bool:
    actions = [
        ...
        (
            args.harden_project_ai_setup,
            "project AI setup hardening",
            lambda: harden_project_ai_setup(project_root, python_executable=sys.executable),
        ),
