...
async def soma_delta() -> str:
    ...
    if state.connected:
        ...
        ok, scene_delta, err = _safe_nexus_result(nexus.scene_delta(gateway.core._last_scene_generation), "scene_delta")
        nexus_payload["scene_delta"] = _safe_text(scene_delta, 2500) if ok else err
        gateway.core._last_scene_generation = state.session_generation
