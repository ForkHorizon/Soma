...
def _confidence_prompt(
    *, source_prompt: str, translation: str, improved_prompt: str, pipeline_status: str, pipeline_warnings: list[str]
) -> str:
...
def _score_confidence_deepseek(result, prompt, model, timeout, started):
    ...
    if not isinstance(decoded, dict) or meta.get("status") != "ok":
        result.update(
            {
                "error": str(meta.get("error") or "DeepSeek confidence check failed."),
...
def _run_confidence_codex(codex_bin, model, reasoning_effort, paths, prompt, timeout, started, result):
    ...
    try:
        return subprocess.run(
            _confidence_command(codex_bin, model, reasoning_effort, paths),
            input=prompt,
            text=True,
