...
def _translate_general_prompt_deepseek(
    ...
    decoded, meta = run_deepseek_json(
        prompt=_deepseek_translation_prompt(source_language, protected),
        schema=_deepseek_translate_schema(),
        model=translator_model,
        timeout=timeout,
        temp_prefix="soma-rus-prompt-deepseek-translate-",
    )
...
def _improve_general_prompt_deepseek(
    ...
    decoded, meta = run_deepseek_json(
        prompt=_deepseek_improvement_prompt(protected),
        schema=_deepseek_improve_schema(),
        model=improver_model,
        timeout=timeout,
        temp_prefix="soma-rus-prompt-deepseek-improve-",
    )
    if not isinstance(decoded, dict):
...
def _deepseek_translation_result(original, source_language, translator_model):
    ...
    result = {
        ...
        "translation_engine": f"deepseek:{translator_model}",
        "translation": "",
...
def _deepseek_translation_error(decoded, translated_protected, protected, original):
    ...
    translation = restore_spans(translated_protected, protected.spans).strip()
    return (
        "DeepSeek translation did not sufficiently normalize Cyrillic text."
