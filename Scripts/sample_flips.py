...
def generate_markdown(sampled: list[dict]) -> str:
    ...
    lines.append("")
    lines.append("Ниже представлены 30 случайных фрагментов (10 терминов, 10 филлеров, 10 формулировок), в которых **Base (`w-greedy`)** и **Candidate (`w-final-candidate`)** выдали разный результат.")
    lines.append("")
    lines.append("## Инструкция по оценке")
    lines.append("1. Для каждого пункта найдите и послушайте аудиофайл (путь указан в строке). Можно открыть через Finder, проиграть `afplay <путь>` в терминале или встроенным плеером.")
    lines.append("2. Прочитайте контекст и сравните два варианта:")
    lines.append("   - **A (Base)**: старая дефолтная модель `w-greedy`")
    lines.append("   - **B (Candidate)**: новый кандидат `w-final-candidate` (`temperature=0.2, best_of=10, initial_prompt=P5`)")
    ...
    lines.append("4. Напишите результаты мне прямо в ответ (например, списком `1: B, 2: B, 3: A, 4: =...` или итоговым счётом `B: 20, A: 5, =: 5`).")
    lines.append("")
    lines.append("---")
    lines.append("")
