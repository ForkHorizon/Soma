...
from rus_to_prompt_stress_results import (
    ...
    canonicalize_confidence_payload,
    confidence_local_checks,
...
def confidence_batch_schema() -> dict[str, Any]:
    return {
        "type": "object",
        "properties": {"results": {"type": "array", "items": _confidence_item_schema()}},
        "required": ["results"],
    }
...
def build_codex_confidence_prompt(case: PromptCase, result: CaseResult, stage: str = "overall") -> str:
    ...
    return (
        "You are a strict prompt-quality referee. Do not use tools. Judge only the JSON payload below.\n"
        + rule
        + stage_rule
        + "Payload:\n"
...
def confidence_chunks_for_group(
    case: PromptCase, results: list[CaseResult], stage: str, batch_size: int
    ...
    with tempfile.TemporaryDirectory(prefix="soma-rus-prompt-confidence-") as tmp:
        ...
        if completed.returncode != 0:
            return _confidence_error(
                "codex", model, stage, completed.stderr or completed.stdout, started, reasoning_effort
            )
        decoded = _extract_json_object(
            output_path.read_text(encoding="utf-8", errors="replace") if output_path.exists() else completed.stdout
        )
    if not isinstance(decoded, dict):
        return _confidence_error(
            "codex", model, stage, "Codex returned invalid confidence JSON.", started, reasoning_effort
        )
...
def score_confidence_with_gemini(
    ...
    started = time.monotonic()
    decoded, meta = _api().run_gemini_json(
    ...
    if decoded is None or meta.get("status") != "ok":
        return _confidence_error(
            ...
            model,
            stage,
            str(meta.get("error") or "Gemini confidence check failed."),
            started,
...
def score_confidence_with_deepseek(
    case: PromptCase, result: CaseResult, model: str, timeout: float, stage: str = "overall"
) -> dict[str, Any]:
    started = time.monotonic()
    decoded, meta = _api().run_deepseek_json(
        prompt=build_codex_confidence_prompt(case, result, stage),
        schema=codex_confidence_schema(),
        model=model,
        timeout=timeout,
        temp_prefix="soma-rus-prompt-deepseek-confidence-",
    )
    if decoded is None or meta.get("status") != "ok":
        return _confidence_error(
            "deepseek",
            model,
            stage,
            str(meta.get("error") or "DeepSeek confidence check failed."),
            started,
            stats=meta.get("stats"),
        )
    return normalize_confidence_payload(
...
def score_confidence_with_local(
    ...
    decoded, meta = _api().run_local_ollama_json(
        prompt=build_codex_confidence_prompt(case, result, stage),
        schema=codex_confidence_schema(),
        model=model,
        timeout=timeout,
    ...
    if decoded is None or meta.get("status") != "ok":
        return _confidence_error(
            "local", model, stage, str(meta.get("error") or "Local confidence check failed."), time.monotonic()
        )
    return normalize_confidence_payload(
        decoded, provider="local", model=model, stage=stage, seconds=float(meta.get("seconds") or 0)
    )
    ...
    for item_id, case, result in items:
        confidences = [
            ...
            or failed_confidence_result(
                model, kwargs["stage"], "Local confidence judge did not return this item.", provider="local"
            )
            for model, batch in zip(local_models, local_results)
        ]
        local_by_item[item_id] = confidences
        reason = hybrid_escalation_reason(
            confidences,
            threshold=kwargs.get("hybrid_local_threshold", DEFAULT_HYBRID_LOCAL_CONFIDENCE_THRESHOLD),
            disagreement_threshold=kwargs.get("hybrid_disagreement_threshold", DEFAULT_HYBRID_DISAGREEMENT_THRESHOLD),
        )
        if reason:
            ...
            final[item_id] = aggregate_local_confidences(
                confidences, model=aggregate_model, stage=kwargs["stage"], batch_item_id=item_id
            )
    ...
    *,
    ...
    for item in decoded["results"]:
        ...
        if item_id in item_ids and item_id not in by_id:
            by_id[item_id] = normalize_confidence_payload(
                item,
                provider=provider,
                model=model,
                stage=stage,
                seconds=batch_seconds / max(len(item_ids), 1),
                reasoning_effort=reasoning_effort,
...
def hybrid_escalation_reason(
    ...
    *,
    ...
    if any(
        confidence.get("status") == "failed" or confidence.get("verdict") == "fail" for confidence in local_confidences
    ):
...
def aggregate_local_confidences(
    ...
    *,
    ...
    numeric = [
        value for value in [confidence_value(confidence) for confidence in local_confidences] if value is not None
    ]
    ...
    status = (
        ...
        if average is None
            ...
            "review"
            if average < threshold or any(confidence.get("status") == "review" for confidence in local_confidences)
            else "ok"
        )
    )
    ...
    return {
        ...
        "status": status,
        "confidence": average,
        "verdict": "pass" if status == "ok" else "review",
        "scores": _average_scores(local_confidences),
        "warnings": warnings[:6],
        "notes": [],
        "seconds": sum(float(c.get("seconds") or 0) for c in local_confidences),
...
def _single_confidence(item: ConfidenceItem, **kwargs: Any) -> dict[str, dict[str, Any]]:
    ...
    if provider == "gemini":
        ...
        confidence = score_confidence_with_codex(
            case, result, model, kwargs["timeout"], kwargs["codex_bin"], stage, kwargs["reasoning_effort"]
        )
...
def _batch_confidence(items: list[ConfidenceItem], **kwargs: Any) -> dict[str, dict[str, Any]] | None:
    ...
    if provider == "gemini":
        ...
        decoded, meta = _api().run_deepseek_json(
            ...
            model=model,
            timeout=kwargs["timeout"],
            temp_prefix="soma-rus-prompt-deepseek-confidence-batch-",
        )
    elif provider == "local":
        decoded, meta = _api().run_local_ollama_json(
            prompt=prompt, schema=confidence_batch_schema(), model=model, timeout=kwargs["timeout"]
        )
    else:
        decoded, meta = _api().run_codex_json(
            prompt=prompt,
            schema=confidence_batch_schema(),
            model=model,
            timeout=kwargs["timeout"],
            codex_bin=kwargs["codex_bin"],
            temp_prefix="soma-rus-prompt-codex-confidence-batch-",
            reasoning_effort=kwargs["reasoning_effort"],
        )
    return parse_batch_confidence_response(
        decoded,
        meta,
        provider=provider,
        model=model,
...
def build_batch_confidence_prompt(items: list[ConfidenceItem], stage: str) -> str:
    ...
    return (
        "You are a strict prompt-quality referee. Do not use tools. Judge each JSON payload item independently.\nPayload:"
        + json.dumps(payload, ensure_ascii=False)
    )
...
def _fallback_confidences(
    ...
    if provider == "off":
        return {
            item_id: aggregate_local_confidences(
                local_by_item[item_id], model=aggregate_model, stage=kwargs["stage"], batch_item_id=item_id
            )
    ...
    return {
        item_id: _attach_or_keep_local(
            item_id, fallback.get(item_id), local_by_item[item_id], reasons[item_id], aggregate_model, kwargs["stage"]
        )
        for item_id, _case, _result in items
    }
...
def _fallback_batch(items: list[ConfidenceItem], provider: str, kwargs: dict[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    fallback_kwargs.update(
        {
            "provider": provider,
            "model": kwargs.get("hybrid_online_model") or kwargs.get("hybrid_gemini_model") or kwargs["model"],
        }
    )
    return score_confidence_batch_with_provider(items, **fallback_kwargs)


def _attach_or_keep_local(
    item_id: str,
        ...
        fallback.update(
            {
                ...
                "local_judges": local,
                "hybrid_escalated": True,
                "hybrid_escalation_reason": reason,
            }
        )
        return fallback
    confidence = aggregate_local_confidences(
        local, model=aggregate_model + " local fallback", stage=stage, batch_item_id=item_id
    )
    ...
    warnings.insert(
        0,
        "Online fallback failed; using conservative local confidence fallback. "
        + str((fallback or {}).get("error") or "online fallback returned no usable result"),
    )
    confidence.update(
        {
            "status": "review",
            "fallback_failed": True,
            "fallback_error_type": (fallback or {}).get("error_type"),
            "warnings": warnings[:6],
            "hybrid_escalated": True,
            "hybrid_escalation_reason": reason,
        }
...
def _average_scores(confidences: list[dict[str, Any]]) -> dict[str, int]:
    ...
    return {
        ...
        if (
            values := [
                float(c.get("scores", {}).get(key))
                for c in confidences
                if isinstance(c.get("scores"), dict) and isinstance(c.get("scores", {}).get(key), (int, float))
            ]
        )
    }
...
def _confidence_error(
    ...
    payload.update(
        {
            key: value
            for key, value in {"reasoning_effort": reasoning_effort, "stats": stats}.items()
