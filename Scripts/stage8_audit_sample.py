...
def build_sample(rows: list[dict], recordings: Path, seed: int,
                 t1_count: int = 40, t2_count: int = 60) -> list[dict]:
    by_tier = {tier: [row for row in rows if row.get("tier") == tier]
               for tier in ("T1", "T2")}
    if len(by_tier["T1"]) < t1_count or len(by_tier["T2"]) < t2_count:
        raise ValueError(f"Need T1={t1_count}, T2={t2_count}; found "
