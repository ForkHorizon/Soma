...
def main():
    ...
    candidate = {r["file"]: r["cleaned"] for r in map(json.loads, (exp / "cleaned-stage7-v1-w-greedy.jsonl").read_text().splitlines())}
    totals, hits = Counter(), Counter()
    for row in human:
        totals[row["label"]] += 1
        if matches(row["label"], candidate[row["file"]], row["human_cleaned"]):
            hits[row["label"]] += 1
    report = {"total": sum(totals.values()), "matched": sum(hits.values()),
              "by_label": {label: {"matched": hits[label], "total": totals[label]}
