...
def main():
    ...
    selected = {"inline": rng.sample(buckets["inline"], inline_count),
    ...
    for index, row in enumerate(output, 1):
        ...
        rows.append({"sample_id": index, "file": row["file"], "audio_path": str(audio),
                     "audio_exists": True, "proposed_text": text, "tier": kind,
                     "confirmed_by": "w-greedy", "audit_status": "pending",
                     "audited_text": None, "notes": None})
