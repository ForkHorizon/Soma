...
def _add_confidence_args(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("--confidence-referee", default="off", choices=["off", "codex", "gemini", "local", "hybrid"])
    ...
    parser.add_argument("--hybrid-confidence-gemini-model", default=os.environ.get("SOMA_RUS_TO_PROMPT_HYBRID_GEMINI_MODEL", "gemini-3-flash-preview"))
    parser.add_argument("--hybrid-confidence-fallback-referee", default="gemini", choices=["gemini", "codex", "off"])
