...
def run_gemini_json(
    *,
    ...
    started = time.monotonic()
    api_key = (
        os.environ.get("SOMA_GEMINI_API_KEY")
    ...
    if not api_key:
        return None, _failed_meta(
            "gemini",
            model,
            "Gemini API key missing. Set it in Soma settings (SOMA_GEMINI_API_KEY) — an AI Studio key, not the AI Pro/CLI login.",
            started,
        )
    full_prompt = (
        prompt
    ...
    return decoded, {
        ...
        "status": "ok",
        "seconds": time.monotonic() - started,
        "stats": wrapper.get("usageMetadata") if isinstance(wrapper, dict) else None,
    }
...
def run_local_ollama_json(
    *,
    ...
    if os.environ.get("SOMA_OLLAMA_WEDGED") == "1" and "-mlx" not in (model or "").lower():
        return None, _failed_meta(
            "local", model, "Ollama GGUF backend unavailable (failed preflight); skipped.", started
        )
...
def translate_with_gemini(
    prompt: str, model: str, timeout: float, gemini_bin: str, model_profile: str
) -> dict[str, Any]:
...
def looks_like_codex_payload_echo(text: str) -> bool:
    ...
    return isinstance(decoded, dict) and any(
        key in decoded for key in ["source_language_hint", "protected_spans", "prompt"]
    )


def _translate_with_provider(
    provider: str, prompt: str, model: str, timeout: float, binary: str, profile: str, effort: str | None
) -> dict[str, Any]:
        ...
        result.update(
            {
                ...
                "translation_tokens": optimizer.estimate_tokens(original, profile),
            }
        )
        return result
    protected = optimizer.protect_spans(original)
    decoded, meta = _call_json_provider(
        provider,
        _translation_prompt(protected, result["source_language"]),
        codex_translate_schema(),
        model,
        timeout,
        binary,
        "translate",
    ...
    result.update(
        {
            ...
            "translation_tokens": optimizer.estimate_tokens(restored, profile),
            f"{provider}_seconds": meta.get("seconds"),
        }
    )
    return result


def _improve_with_provider(
    provider: str, prompt: str, model: str, timeout: float, binary: str, profile: str, effort: str | None
) -> dict[str, Any]:
    ...
    decoded, meta = _call_json_provider(
        provider, _improve_prompt(protected), codex_improve_schema(), model, timeout, binary, "improve", effort
    )
    improved_protected = str((decoded or {}).get("improved_prompt") or "").strip()
    warnings = list((decoded or {}).get("warnings") or []) if isinstance((decoded or {}).get("warnings"), list) else []
    if not isinstance(decoded, dict):
        return _degraded_result(
            result, translation, warnings + [_provider_failure(provider, "improvement", decoded, meta)], "", profile
        )
    if not _provider_status_ok(decoded) or not improved_protected or looks_like_codex_payload_echo(improved_protected):
        ...
        if looks_like_codex_payload_echo(improved_protected):
            extra = (
                f"{_provider_label(provider)} improvement echoed the control payload instead of improving the prompt."
            )
    ...
    if validation_error:
        return _degraded_result(
            result,
            translation,
            warnings + [f"{_provider_label(provider)} improvement failed validation: " + validation_error],
            "",
            profile,
        )
    result.update(
        {
            ...
            "warnings": warnings,
            "protected_spans_count": len(protected.spans),
            "improved_prompt_tokens": optimizer.estimate_tokens(improved, profile),
            f"{provider}_seconds": meta.get("seconds"),
        }
    )
    return result
...
def _call_json_provider(
    ...
    api = _api()
    if provider == "gemini":
        return api.run_gemini_json(
            prompt=prompt,
            schema=schema,
            model=model,
            timeout=timeout,
            gemini_bin=binary,
            temp_prefix=f"soma-rus-prompt-gemini-{stage}-",
        )
    if provider == "deepseek":
        return api.run_deepseek_json(
            prompt=prompt, schema=schema, model=model, timeout=timeout, temp_prefix=f"soma-rus-prompt-deepseek-{stage}-"
        )
    return api.run_codex_json(
        prompt=prompt,
        schema=schema,
        model=model,
        timeout=timeout,
        codex_bin=binary,
        temp_prefix=f"soma-rus-prompt-codex-{stage}-",
        reasoning_effort=effort or DEFAULT_CODEX_STAGE_REASONING_EFFORT,
    )
...
def _translation_result(model: str, provider: str, original: str) -> dict[str, Any]:
    return {
        ...
        "translation_engine": f"{provider}:{model}",
        "translation": "",
...
def _translation_prompt(protected: Any, source_language: str) -> str:
    return (
        "You are a precise technical translator. Do not use tools. Return JSON only. Set status exactly to ok when the translation succeeds, otherwise failed. Preserve placeholders exactly.\nSource language hint: "
        + source_language
        + "\nProtected prompt:\n<<<PROMPT\n"
        + protected.text
        + "\nPROMPT>>>"
...
def _translation_failure(
    decoded: dict[str, Any] | None, translated: str, protected: Any, original: str, provider: str, meta: dict[str, Any]
) -> str | None:
...
def _degraded_result(
    result: dict[str, Any], fallback: str, warnings: list[str], extra: str, profile: str
) -> dict[str, Any]:
    if extra:
    ...
    result.update(
        {
            ...
            "warnings": warnings,
            "improved_prompt_tokens": optimizer.estimate_tokens(fallback, profile),
        }
    )
    return result
...
def _run_stage_process(
    cmd: list[str], prompt: str, timeout: float, env: dict[str, str] | None = None, cwd: str | None = None
...
def _ollama_payload(prompt: str, schema: dict[str, Any], model: str) -> dict[str, Any]:
    ...
    return {
        ...
        "options": {
            ...
            "seed": int(os.environ.get("SOMA_LOCAL_SEED", "0")),
        },
    }
