...
def run(kind, rows):
    ...
    if kind == "parakeet":
        ...
        for row in rows:
            ...
            with torch.no_grad(): generated = model.generate(**inputs)
        ...
        for row in rows:
            ...
            inputs = proc(text=[prompt], audio=[audio], sampling_rate=16000,
                          return_tensors="pt", padding=True)
            with torch.no_grad(): ids = model.generate(**inputs, max_new_tokens=768)
        ...
        model = Wav2Vec2ForCTC.from_pretrained(repo, **kwargs,
                                               **({"ignore_mismatched_sizes": True} if kind == "mms" else {}))
        for row in rows:
        ...
        for row in rows:
            ...
            with wave.open(row["audio"]) as handle:
                while chunk := handle.readframes(4000): rec.AcceptWaveform(chunk)
...
def main() -> int:
    ...
    for row, text, words, version in run(kind, rows):
        print(json.dumps({"id": row["id"], "file": row["file"], "text": (text or "").strip(),
                          "words": words, "version": version}, ensure_ascii=False), flush=True)
    return 0
