...
from rus_to_prompt_stress_providers import (
    improve_with_codex,
    improve_with_deepseek,
    improve_with_gemini,
...
def _run_translation_mode(cases, translators, args, file, results, total_operations):
    ...
    for case in cases:
        for translator in translators:
            ...
            if existing is not None:
                emit_resume_skip(
                    case, "translation_resume", translator, TRANSLATION_ONLY_ANALYZER_MODEL, operation, total_operations
                )
                continue
            translation, translation_seconds = _run_translation_stage(
                case, translator, args, operation, total_operations
            )
            provider = provider_for_stage_model(translator, args.translator_provider)
            result = build_translation_only_result(
                case, translator, TRANSLATION_ONLY_ANALYZER_MODEL, provider, "none", translation, translation_seconds
            )
...
def _run_staged_mode(cases, translators, analyzers, args, file, results, total_operations):
    ...
    for case in cases:
        ...
        for translator in translators:
            ...
            if existing is not None:
                ...
                translation_rows.append(
                    (translation_payload_from_result(existing), existing.translation_seconds or 0.0, existing)
                )
        ...
        operation = _run_staged_improvers(
            case, analyzers, translation_rows, args, file, results, operation, total_operations
        )


def _run_staged_improvers(
    case, analyzers, translation_rows, args, file, results, operation: int, total_operations: int
) -> int:
    ...
    for analyzer in analyzers:
        ...
        if best is None:
            ...
            write_result(file, results, result, operation, total_operations)
            cooldown(
                args, case, result.translator_model, analyzer, operation, total_operations, "improver stage finished"
            )
            continue
        else:
            translation_payload, translation_seconds, translation_result = best
            existing = find_existing_result(
                results, case.id, translation_result.translator_model, analyzer, args.benchmark_mode
            )
            if existing is not None:
                emit_resume_skip(
                    case, "improver_resume", translation_result.translator_model, analyzer, operation, total_operations
                )
                continue
            result = _run_improver_from_translation(
                case,
                translation_payload,
                translation_seconds,
                translation_result,
    ...
    score_and_attach_confidence_batches(
        case, [("improve", pending_improver_results), ("overall", pending_improver_results)], args, total_operations
    )
...
def _emit_best_translation(case, best, operation: int, total_operations: int) -> None:
    if best is None:
        print(
            progress_event_line(
                event="best_translation_selected",
                ...
                status="rejected",
                reason="No translation passed the confidence gate.",
            ),
            flush=True,
        )
        return
...
def _run_one(case, translator: str, analyzer: str, args, operation: int, total_operations: int):
    ...
    _emit_stage_complete(
        case, "translating", translator, analyzer, operation, total_operations, translation.get("status")
    )
    ...
    _emit_stage_complete(
        case, "analyzing", translator, analyzer, operation, total_operations, (improve or {}).get("status") or "skipped"
    )
...
def _run_improver_from_translation(
    case, translation_payload, translation_seconds, translation_result, analyzer, args, operation, total_operations
):
