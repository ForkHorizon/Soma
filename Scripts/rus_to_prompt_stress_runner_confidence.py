...
def score_and_attach_confidence(case, result, stage: str, args, operation: int, total_operations: int) -> None:
    ...
    try:
        ...
        confidence = by_id.get(item_id) or failed_confidence_result(
            ...
            stage,
            "Confidence judge did not return a result.",
            args.confidence_reasoning_effort,
            provider=args.confidence_referee,
        )
    except Exception as exc:
        confidence = failed_confidence_result(
            args.confidence_model, stage, str(exc), args.confidence_reasoning_effort, provider=args.confidence_referee
        )
...
def score_and_attach_confidence_batch(
    case, operation_results: list[tuple[int, Any]], stage: str, args, total_operations: int
) -> None:
    ...
    for batch_index, batch in enumerate(batches, start=1):
        ...
        print(
            progress_event_line(
                ...
                confidence_item_ids=[item_id for item_id, _case, _result in items],
                confidence_model_refs=confidence_model_refs([result for _operation, result in batch]),
                **scope,
            ),
            flush=True,
        )
        ...
        print(
            progress_event_line(
                ...
                batch_index=batch_index,
                batch_total=len(batches),
                status=confidence_batch_status(attached),
                confidence=confidence_batch_average(attached),
                confidence_model=args.confidence_model,
                confidence_item_ids=[item_id for item_id, _case, _result in items],
                confidence_model_refs=confidence_model_refs([result for _operation, result in batch]),
                **scope,
...
def score_and_attach_confidence_batches(
    case, stage_operation_results: list[tuple[str, list[tuple[int, Any]]]], args, total_operations: int
) -> None:
...
def _score_and_attach_hybrid_model_major(
    case, stage_operation_results: list[tuple[str, list[tuple[int, Any]]]], args, total_operations: int
) -> None:
    ...
    for judge_index, model in enumerate(local_models, start=1):
        for batch_info in _iter_confidence_batches(batches_by_stage):
            _score_local_judge_batch(
                case, batch_info, model, judge_index, len(local_models), args, total_operations, state
            )
...
def _confidence_stage_batches(
    case, stage_operation_results: list[tuple[str, list[tuple[int, Any]]]], args
) -> list[dict[str, Any]]:
...
def _score_local_judge_batch(
    ...
    missing = [
        ...
        if _confidence_state_key(item_id, model) not in state["local_judges"]
    ]
    status = "cached" if not missing else "running"
    print(
        progress_event_line(
            event="confidence_batch_start",
            stage=batch_info["stage_label"],
            case_id=case.id,
            category=case.category,
            operation_index=batch_info["operation"],
            total_operations=total_operations,
            batch_size=len(items),
            batch_index=batch_info["batch_index"],
            batch_total=batch_info["batch_total"],
            status=status,
            confidence_model=model,
            confidence_judge_index=judge_index,
            confidence_judge_total=judge_total,
            confidence_item_ids=[item_id for item_id, _case, _result in items],
            confidence_model_refs=batch_info["refs"],
            **batch_info["scope"],
        ),
        flush=True,
    )
    ...
    if missing:
        try:
            by_id = score_confidence_batch_with_provider(
                missing, **_local_confidence_kwargs(args, batch_info["stage"], model)
            )
        except Exception as exc:
            by_id = {
                item_id: failed_confidence_result(model, batch_info["stage"], str(exc), provider="local")
                for item_id, _case, _result in missing
            }
        for item_id, _case, _result in missing:
            confidence = by_id.get(item_id) or failed_confidence_result(
                model, batch_info["stage"], "Local confidence judge did not return this item.", provider="local"
            )
...
def _aggregate_hybrid_confidences(
    batches_by_stage: list[dict[str, Any]], local_models: list[str], args, state: dict[str, Any]
    ...
    for batch_info in _iter_confidence_batches(batches_by_stage):
        ...
        for item_id, case, result in batch_info["items"]:
            confidences = [
                ...
                or failed_confidence_result(
                    model, stage, "Local confidence judge did not return this item.", provider="local"
                )
            ...
            reason = hybrid_escalation_reason(
                confidences,
                threshold=args.hybrid_confidence_local_threshold,
                disagreement_threshold=args.hybrid_confidence_disagreement_threshold,
            )
            if reason:
                ...
                final[item_id] = aggregate_local_confidences(
                    confidences, model=aggregate_model, stage=stage, batch_item_id=item_id
                )
...
def _fallback_confidences_model_major(
    items,
    local_by_item: dict[str, list[dict[str, Any]]],
    reasons: dict[str, str],
    aggregate_model: str,
    ...
    if provider == "off":
        return {
            item_id: aggregate_local_confidences(
                local_by_item[item_id], model=aggregate_model, stage=stage, batch_item_id=item_id
            )
            for item_id, _case, _result in items
        }
    fallback_kwargs = confidence_kwargs(args, stage)
    fallback_kwargs.update(
        {"provider": provider, "model": args.hybrid_confidence_online_model or args.confidence_model}
    )
    try:
        fallback = score_confidence_batch_with_provider(items, **fallback_kwargs)
    except Exception as exc:
        fallback = {
            item_id: failed_confidence_result(fallback_kwargs["model"], stage, str(exc), provider=provider)
            for item_id, _case, _result in items
        }
    return {
        item_id: _attach_or_keep_local(
            item_id, fallback.get(item_id), local_by_item[item_id], reasons[item_id], aggregate_model, stage
        )
        for item_id, _case, _result in items
    }


def _attach_hybrid_confidences(
    case, batches_by_stage: list[dict[str, Any]], final: dict[str, dict[str, Any]], args, total_operations: int
) -> None:
    for batch_info in _iter_confidence_batches(batches_by_stage):
        attached = []
        for _operation, result in batch_info["batch"]:
            item_id = confidence_item_id(result, batch_info["stage"])
            confidence = final.get(item_id) or failed_confidence_result(
                args.confidence_model,
                batch_info["stage"],
                "Confidence judge did not return a result.",
                args.confidence_reasoning_effort,
        ...
        print(
            progress_event_line(
                ...
                operation_index=batch_info["operation"],
                total_operations=total_operations,
                batch_size=len(batch_info["items"]),
                batch_index=batch_info["batch_index"],
                batch_total=batch_info["batch_total"],
                status=confidence_batch_status(attached),
                confidence=confidence_batch_average(attached),
                confidence_model=args.confidence_referee,
                confidence_item_ids=[item_id for item_id, _case, _result in batch_info["items"]],
                confidence_model_refs=batch_info["refs"],
                **batch_info["scope"],
            ),
            flush=True,
...
def confidence_batch_status(confidences) -> str:
    statuses = {
        canonical_confidence_status(confidence) or "unknown"
...
def confidence_batch_average(confidences) -> float | None:
    numeric = [
        value
        for confidence in confidences
        if isinstance(confidence, dict) and (value := confidence_value(confidence)) is not None
    ]
...
def _probe_online_provider(provider: str, model: str, args) -> tuple[bool, str]:
    ...
    try:
        if provider == "gemini":
            _decoded, meta = api.run_gemini_json(
                prompt=_PREFLIGHT_PROMPT,
                schema=_PREFLIGHT_SCHEMA,
                model=model,
                timeout=_PREFLIGHT_TIMEOUT,
                gemini_bin=args.gemini_bin,
                temp_prefix="soma-preflight-gemini-",
            )
        elif provider == "deepseek":
            _decoded, meta = api.run_deepseek_json(
                prompt=_PREFLIGHT_PROMPT,
                schema=_PREFLIGHT_SCHEMA,
                model=model,
                timeout=_PREFLIGHT_TIMEOUT,
                temp_prefix="soma-preflight-deepseek-",
            )
        elif provider == "codex":
            _decoded, meta = api.run_codex_json(
                prompt=_PREFLIGHT_PROMPT,
                schema=_PREFLIGHT_SCHEMA,
                model=model,
                timeout=_PREFLIGHT_TIMEOUT,
                codex_bin=args.codex_bin,
                temp_prefix="soma-preflight-codex-",
                reasoning_effort=args.confidence_reasoning_effort,
            )
        else:
            return True, ""
    ...
    for provider, model in targets:
        ...
        print(
            progress_event_line(
                ...
                reason=None if ok else reason[:200],
    ...
    if targets and len(dead) >= len(targets):
        print(
            progress_event_line(
                ...
                stage="preflight",
                status="degraded",
                reason="All online confidence providers failed preflight; confidence will fail until one is restored.",
            ),
            flush=True,
        )
    args._dead_confidence_providers = dead
...
def preflight_local_backend(translators: list[str], analyzers: list[str]) -> bool:
    ...
    candidates = [
        m
        for m in list(translators) + list(analyzers)
    ...
    if not ok:
        ...
        print(
            progress_event_line(
                ...
                stage="preflight",
                status="degraded",
                reason=f"Ollama GGUF runner not responding (probe {probe_model}); non-mlx local models skipped this run. Restart Ollama to recover.",
            ),
            flush=True,
        )
    return not ok
...
def emit_translation_gate(case, result, args, operation: int, total_operations: int) -> None:
    ...
    confidence = (
        (result.translation_confidence or {}).get("confidence")
        if isinstance(result.translation_confidence, dict)
        else None
    )
    print(
        progress_event_line(
...
def best_translation(rows, args):
    ...
    for position, (translation_payload, translation_seconds, result) in enumerate(rows):
        ...
        if args.confidence_referee == "off":
            accepted.append(
                (
                    translation_selection_score(result, translation_seconds, 1.0, position),
                    translation_payload,
                    translation_seconds,
                    result,
                )
        ...
        if translation_confidence_allows_improve(confidence, args.translation_confidence_threshold):
            ...
            accepted.append(
                (
                    translation_selection_score(result, translation_seconds, float(value or 0.0), position),
                    translation_payload,
                    translation_seconds,
                    result,
                )
...
def translation_selection_score(result, translation_seconds: float | None, confidence: float, position: int) -> tuple:
    ...
    local_values = [
        confidence_value(judge) for judge in confidence_payload.get("local_judges", []) if isinstance(judge, dict)
    ]
...
def rejected_improver_result(case, analyzer, args):
    payload = {
        ...
        "warnings": ["No translation passed the staged confidence gate."],
    }
    provider = provider_for_stage_model(analyzer, args.analyzer_provider)
    return build_translation_rejected_result(
        case,
        "none",
        analyzer,
        "none",
        ...
        payload,
        0.0,
        "No translation passed the staged confidence gate; skipped improver stage.",
    )
...
def cooldown(
    args, case, translator: str | None, analyzer: str | None, operation: int, total_operations: int, reason: str
) -> None:
...
def wait_if_paused(
    args, case, translator: str | None, analyzer: str | None, operation: int, total_operations: int
) -> None:
    paused = False
    while True:
        control = read_control_file(args.control_file)
